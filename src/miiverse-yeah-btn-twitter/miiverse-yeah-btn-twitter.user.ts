...
const setInput = async (el: HTMLElement) => {
  ...
  dataTransfer.setData("image/jpeg", blob.toString());
...
function handleTweetFillInput(targetEl: HTMLElement) {
...
function handleTweetSendBtn() {
  let sendTweetBtn = document.querySelector(
    "[data-testid='tweetButton']"
  ) as HTMLElement;
  if (!sendTweetBtn) {
    sendTweetBtn = document.querySelector(
  ...
  for (const mutation of mutationsList) {
    if (mutation.type === "childList") {
      ...
      for (const node of nodes) {
        if (node && typeof (node as HTMLElement).querySelector === "function") {
          let els = (node as HTMLElement).querySelectorAll(
            "[role='group']:not(:has(.yeah-btn))"
          );
          if (els.length === 0) {
            const contElem = (node as HTMLElement).querySelector(
            ...
            if (contElem) {
              ...
              if (waitForPopup) {
                ...
                handleTweetFillInput(contElem as HTMLElement);
              }
            }
          }
            ...
            if (el && el.childElementCount > 3) {
              ...
              yeahBtn.addEventListener("click", () => {
                if (isSmall) {
                  ...
                  handleTweetFillInput(el as HTMLElement);
                }
              });
          ...
          if (waitForImage) {
            if (
              ...
              (node as HTMLImageElement).src.includes("blob:")
            ) {
              handleTweetSendBtn();
            }
          }
